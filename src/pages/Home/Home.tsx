...
const Home = () => {
    ...
    useEffect(() => {
        const fetchRatings = async () => {
            try {
                const res = await axios.get<RatingItem[]>(API_URL + "/public/users-rating");
                setRating(res.data);
            } catch (err: any) {
                if (err.response) {
    ...
    return (
        <div className=" flex flex-col items-center py-12 px-4">
            <div
                className="bg-white/90 backdrop-blur-lg border border-[#e7d3c8] text-center py-12 px-8 rounded-3xl shadow-xl w-full max-w-3xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {rating.length === 0 ? (
                            ...
                            rating.map((item, index) => (
                                ...
                                >
                                    <div
                                        className="bg-gradient-to-r from-[#af8773] to-[#c2937b] text-white w-12 h-12 flex items-center justify-center rounded-full font-bold shadow-lg mb-4">
                                        {index + 1}
                                    </div>
